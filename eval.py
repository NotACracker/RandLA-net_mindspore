...
def run_eval(arg):
    ...
    context.set_context(mode=context.PYNATIVE_MODE, device_target=arg.device_target, device_id=arg.device_id)

    logger = get_logger(arg.outputs_dir, arg.rank)
    ...
    val_loader = val_ds.batch(batch_size=arg.batch_size,
    ...
    for _, ckpt in enumerate(ckpts):
        #load current ckpt
        logger.info('load ckpt from: %s', str(ckpt))
    ...
    for i in range(len(best_names)):
        ...
        write_ply(os.path.join(arg.outputs_dir, 'val_preds', name), [preds, labels], ['pred', 'label'])
...
if __name__ == "__main__":
    ...
    expr.add_argument('--batch_size', type=int, help='val batch size', default=32)
    ...
    misc.add_argument('--device_target', type=str, help='Ascend or GPU', default='GPU')

    misc.add_argument('--device_id', type=int, help='Ascend/GPU id to use', default=0)
    ...
    # val pred path
    if not os.path.exists(os.path.join(args.outputs_dir, 'val_preds')):
        os.makedirs(os.path.join(args.outputs_dir, 'val_preds'))
